const { requireEntity, listEntities, linksFor } = require('./entities');
...
const { definition } = require('./entities');
async function relatedItems(db, orgId, type, id) {
  ...
  async function include(otherType, otherId) {
    if (!Object.hasOwn(require('./entities').entities, otherType) || !/^[1-9]\d*$/.test(String(otherId))) return;
